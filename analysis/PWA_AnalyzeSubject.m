...
%add time between word offset and saccade onset
d.wordOffset_SaccStart = d.edfTStimOff - d.saccStartTime;
...
%                               %there was 1 clear saccade to target, word onset before saccade onset, word offset before saccade offset, and less than 10% of word duration overlapped with saccade                                
%d.goodSacTrial(finishedTrials) = d.offlineGoodSac(finishedTrials) & d.wordOnset_SaccStart(finishedTrials)<=0 & d.wordOffset_SaccEnd(finishedTrials)<0 & d.propWordDuringSacc(finishedTrials)<0.10;

%or could say, wod offet before saccade onset 
d.goodSacTrial(finishedTrials) = d.offlineGoodSac(finishedTrials) & d.wordOffset_SaccStart(finishedTrials)<=0;

%for neutral trials, that stuff doesn tmatter
...
wordDurs = d.edfTStimOff(goodTrials) - d.edfTStimOn(goodTrials);
r.meanWordDur = mean(wordDurs);
r.stdWordDur = std(wordDurs);
